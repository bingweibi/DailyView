...
public class ResponseHandleUtility {
    ...
    public static List<ZhiHuDailyHot.Recent> handleZhiHuDailyHot(String responseText) throws JSONException {

        JSONObject jsonObject = new JSONObject(responseText);
        JSONArray jsonArray = jsonObject.getJSONArray("recent");
        List<ZhiHuDailyHot.Recent> recentList = new ArrayList<>();
        for (int i=0;i<jsonArray.length();i++){
            String element = jsonArray.get(i).toString();
            ZhiHuDailyHot.Recent recent = new Gson().fromJson(element,ZhiHuDailyHot.Recent.class);
